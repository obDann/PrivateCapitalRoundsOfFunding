...
```{r}
ggplot(pt1, aes(x = year, y = raised_amount, color = investment_type)) + geom_point() + scale_y_continuous(labels = scales::comma) + geom_smooth(method = "lm", fill = NA) + ggtitle("Summation of amount raised per investment type (Graced)") + theme(text = element_text(size = 15), plot.title=element_text(hjust = 0.5))  + ylab("Sum of amount raised") + xlab("Year") + labs(colour = "Investment Type")
```
...
ggplot(for_comp, aes(y=raised_amount, x=situation, fill = situation)) + 
    geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma) + theme(text = element_text(size = 15), plot.title=element_text(hjust = 0.5, size = 12), axis.text.x = element_blank(), axis.ticks.x = element_blank()) + guides(fill = guide_legend(title="Situation")) + ggtitle("Summation of money raised per situation")
```
