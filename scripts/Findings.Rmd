...
#canada_funding %>% 
{my_pal = colorBin(c("#3B5284", "#00FF00"), domain = 1:7, bins = 7)
...
```{r}
my_pal = colorBin(c("#FDBCB4", "#333399"), domain = 1:6, bins = 5)
...
start_rng = c(the_min, the_min + floor(interval*1) + 1, the_min + floor(interval*2) + 1, the_min + floor(interval*3) + 1, the_min + floor(interval*4) + 1)

end_rng = c(the_min + floor(interval*1), the_min + floor(interval*2), the_min + floor(interval*3), the_min + floor(interval*4), max(NA_funding$amount_raised))
...
leaflet() %>% addProviderTiles(providers$Stamen.TonerLite) %>% addCircleMarkers(data = with_pals, lng = ~lon, lat = ~lat,
  ...
  addLegend(data = with_pals, colors = pspal, title = "Summation of amount raised per city", labels = alt_pal$range_str)
```

```{r}
with_pals %>% select(amount_raised, start_rng, end_rng, ciaco)
```
